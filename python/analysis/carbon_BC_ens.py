...
def plot_wurst_BC(ax, model,marker, addition):
    ...
    for mt,c,p in zip(BC_methods,color,position):
        ...
        ax7.scatter(p+addition,df_CTotal_diff,color=c,marker=marker)
        ax8.scatter(p+addition,df_NEE_diff,color=c,marker=marker)
...
def plot_wurst_ens(method, selection, color, position, marker):
    ...
    for sme, sma in zip(selection_methods, selection_markers):
        if sme == 'full':
            ax7.scatter(position,df_CTotal_diff[sme],color=color,
                        marker=sma,facecolors='none')
            ax8.scatter(position,df_NEE_diff[sme],color=color,
                        marker=sma,facecolors='none')
        else:
            if method == 'Weighted':
                pass
                ...
                if sme in ('skill', 'independence'):
                    ...
                    addition=0.2
                ax7.scatter(position+addition,df_CTotal_diff[sme],
                            color=color,marker=sma)
                ax8.scatter(position+addition,df_NEE_diff[sme],
                            color=color,marker=sma)
...
title_name = ['EC-Earth3-Veg', 'INM-CM4-8', 'KIOST-ESM', 'MPI-ESM1-2-HR',
              'NorESM2-MM', 'Ensemble average', 'Average $\mathrm{C_{Total}}$',
              'IAV NBP']
